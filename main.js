...
const SURPRISES = [
    ...
    [cite_start]["An edgy and futuristic name for a cyberpunk productivity app", "App", "Futuristic", "English"], [cite: 186]
    [cite_start]["A wholesome name for a cozy coffee shop in a rainy city", "Place", "Wholesome", "English"], [cite: 186]
    [cite_start]["A bold and cryptic name for an underground hacker forum", "Platform", "Dark", "English"], [cite: 186]
    [cite_start]["A magical and whimsical name for a children’s toy line", "Product", "Whimsical", "English"], [cite: 187]
];
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Initialize Auth Elements now that HTML is loaded
...
async function generateName() {
    ...
    [cite_start]disableButtons(); [cite: 242]
    [cite_start]refinedOutputs.classList.remove("visible-section"); [cite: 243]
    [cite_start]refinedOutputs.classList.add("hidden-section"); [cite: 243]
    [cite_start]refineSection.classList.remove("visible-section"); [cite: 244]
    [cite_start]refineSection.classList.add("hidden-section"); [cite: 244]
    [cite_start]refineBtn.classList.remove("visible-section"); [cite: 244]
    ...
    try {
        ...
        if (!response.ok) {
            [cite_start]const errorData = await response.json(); [cite: 246]
        ...
        [cite_start]enableButtons(); [cite: 255]
    }
}
...
async function refineNames() {
    ...
    [cite_start]disableButtons(); [cite: 261]
    ...
    try {
        ...
        [cite_start]refinedOutputs.classList.remove("visible-section"); [cite: 268]
        ...
        [cite_start]enableButtons(); [cite: 269]
    }
}
...
[cite_start]async function showHistoryDetails(id) { [cite: 328, 329]
    ...
    try {
        [cite_start]const response = await fetch(`${BACKEND_URL}/history`, { headers: { "Authorization": `Bearer ${token}` } }); [cite: 331]
