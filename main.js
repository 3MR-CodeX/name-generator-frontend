...
    if (!BACKEND_URL) { // Removed !API_KEY check
...
    if (!instruction || !BACKEND_URL) { // Removed !API_KEY check
...
    try {
        const response = await fetch(`${BACKEND_URL}/history`); // Removed headers: { "x-api-key": API_KEY }
        if (!response.ok) throw new Error(await response.text());
